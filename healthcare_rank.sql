WITH combined_data AS (
    -- Combine all failure reasons and calculate their frequencies for the 'healthcare' table only
    SELECT 'no_budget' AS failure_reason, SUM(IFNULL(no_budget, 0)) AS frequency FROM healthcare
    UNION ALL
...
total AS (
    -- Calculate the total frequency for the 'healthcare' table
